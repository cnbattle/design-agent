...
import { AuthStorage, createAgentSession, DefaultResourceLoader, defineTool, getAgentDir, ModelRegistry, SessionManager, } from "@earendil-works/pi-coding-agent";
class VirtualFS {
    files;
    _changed = new Set();
...
async function handleRun(req) {
    ...
    const writeTool = defineTool({
        ...
        execute: async (_id, params) => {
            ...
            if (danger) {
                return { content: [{ type: "text", text: `BLOCKED: ${danger} detected in ${params.path}. Remove it.` }], details: {} };
            }
    ...
    const editTool = defineTool({
        ...
        execute: async (_id, params) => {
            ...
            if (danger) {
                return { content: [{ type: "text", text: `BLOCKED: ${danger} detected in replacement. Remove it.` }], details: {} };
            }
